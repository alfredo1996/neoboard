...
| **Local** | `neoboard setup --mode local` | App runs locally with Node.js. Setup installs dependencies, writes `app/.env.local`, and runs migrations, but it does not start any databases: PostgreSQL and Neo4j must already be running (for example `docker compose up -d`). Then run `neoboard dev` to start the dev server. It restarts the databases in Docker if they have stopped. |
...
| `neoboard logs` | Tail the PostgreSQL and Neo4j container logs. |
...
Open `http://localhost:3000` and log in with the demo accounts it prints — [Tour NeoBoard with demo data](/start-here/tour) shows what got seeded.

## Next Steps

<CardGrid>
  <LinkCard title="Tour NeoBoard with demo data" href="/start-here/tour" />
  <LinkCard title="Development setup" href="/extend/setup" />
