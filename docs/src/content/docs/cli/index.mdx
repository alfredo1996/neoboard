...
| Command | Description |
|---------|-------------|
| `neoboard env generate` | Generate `app/.env.local` with fresh secrets. |
| `neoboard env validate` | Check all required env vars are set. |
| `neoboard env list` | Show all known env vars with set/unset status. |
| `neoboard env get <key>` | Get a single env var value. |
| `neoboard env set <key> <value>` | Set a single env var in `.env.local`. |
