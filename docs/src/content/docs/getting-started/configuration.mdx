...
| `MIGRATE_ON_START` | Apply pending schema migrations on boot. The production Docker image sets this to `1` (on); set `0` to skip (see [Deployment Checklist → Database migrations](/administration/deployment-checklist#database-migrations)). | `1` (Docker image) |
