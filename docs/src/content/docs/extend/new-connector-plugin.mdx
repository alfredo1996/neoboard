...
The built-in Neo4j and PostgreSQL connectors are plugins too: each lives in its own directory under `connection/src/` and is registered with `registry.register()` in `connection/src/connector-registry.ts`, exactly as in the Quick Start. Adding a connector to NeoBoard itself is a PR that adds the directory and the registration line. The connection package has no UI and no React — whatever the connection form needs is declared through the plugin's form fields, not by editing `component/`. Integration tests for a core connector run against a real database (see [Testing](/extend/testing)). To ship the connector as an npm package instead, see [External plugins](/extend/external-plugins) and the [MongoDB example](/extend/mongodb-connector).
