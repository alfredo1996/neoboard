---
title: PR Workflow
description: Branch naming, labels, review process, and CI requirements.
sidebar:
  order: 12
---

## Branch Naming

Branch from `dev`, not `main`. While a `release/X.Y` branch is active, branch from that release branch instead:

```
feat/issue-<N>-<slug>    # New features
fix/issue-<N>-<slug>     # Bug fixes
chore/<slug>             # Maintenance
```

## Commit Messages

Follow [Conventional Commits](https://www.conventionalcommits.org/):

```
type(scope): description

feat(app): add API key authentication
fix(component): correct pie chart tooltip overflow
test(connection): add Neo4j pool timeout tests
chore: update dependencies
```

## Pull Requests

- PRs target the branch you branched from: `dev`, or the active `release/X.Y`, which merges into `dev`. `main` holds stable releases
- Link the issue: `Closes #N` in the PR body
- Add labels: type (`feat`, `fix`) + package (`pkg:app`) + area (`area:api`)
- Set the correct milestone

## CI Requirements

CI runs these checks on every PR:

1. **Lint & Type Check** — ESLint + `tsc --noEmit`
2. **Unit Tests** — Vitest (app, component) + Jest (connection)
3. **E2E Tests** — Playwright with real databases
4. **SonarCloud** — Coverage ≥ 80%, duplication ≤ 3%, no bugs/vulnerabilities

## Code Review

- **CodeRabbit** automatically reviews PRs — address or dismiss all suggestions
- **SonarQube** quality gate must pass
- Nothing in GitHub enforces these checks: there is no branch protection and there are no rulesets. A maintainer merges your PR once CI is green and the review feedback has been addressed. The maintainer's merge is the only sign-off.
