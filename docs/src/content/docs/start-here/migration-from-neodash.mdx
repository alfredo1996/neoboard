...
| Treemap (`treeMap`, `treemap`) | — | Not shipped in NeoBoard: the report is skipped and listed in the import notes |
...
| Circle Packing (`circle_packing`, `circlePacking`) | — | Not shipped in NeoBoard: the report is skipped and listed in the import notes |
...
| Widget templates | No | Yes (Widget Library) |
...
- **Authentication** — NeoDash relies on Neo4j auth; NeoBoard has its own user management, plus OIDC [single sign-on](/security/sso) in the enterprise edition
...
for file in neodash-exports/*.json; do
  curl -X POST http://localhost:3000/api/dashboards/import \
    ...
    -d "{
      \"payload\": $(cat "$file"),
      \"connectionMapping\": {
        \"neodash-default\": \"your-neoboard-connection-id\"
      }
    }"
  echo " <- imported $file"
done
