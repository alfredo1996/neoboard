...
5. **Restart with the new key.** Each connection owner opens Settings → Connections → Edit and re-enters the full config: URI, username, password and database. Only the owner can edit a connection, so reassign orphaned ones first. Until then those connections fail their test and their widgets error. Do not call the key-rotation endpoint until every connection has been re-entered: it fails and rolls back on an empty blob.
