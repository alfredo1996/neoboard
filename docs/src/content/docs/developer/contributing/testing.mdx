...
|-------|------|---------|---------|
| Pure functions | Vitest (`unit`, node) | `app/` | chart-plugin-registry, date-utils, query-hash |
...
| UI components | Vitest + RTL (`component`, jsdom) | `app/` | Error states, loading states, branch coverage E2E cannot reach |
| Integration | Jest + Testcontainers | `connection/` | Real DB queries |
| Full flows | Playwright | `app/` | Real rendering, real data |

**Two Vitest projects in `app/`** (`app/vitest.config.ts`): `unit` runs `.test.ts` in a node
environment for pure logic, API routes and stores; `component` runs `.test.tsx` in jsdom with
`@testing-library/react`. Reach for the jsdom project when a branch is hard to hit end to end —
error and loading states, edge cases — and leave whole user journeys to Playwright.
