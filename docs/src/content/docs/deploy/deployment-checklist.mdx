...
- [ ] Version pinned — an explicit image tag (e.g. `ghcr.io/…/neoboard:v1.3.0`, **not** `:latest`) via `NEOBOARD_IMAGE` for Docker Compose or the manifest for Kubernetes, a release tag checkout for a build you run yourself — so deploys are reproducible and an upgrade cannot cross a major version unintentionally
