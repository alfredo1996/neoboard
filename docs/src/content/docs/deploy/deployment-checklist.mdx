...
Use this checklist before deploying NeoBoard to production or to a new customer. It applies to every production option — Docker Compose, a Node.js process under systemd, or Kubernetes ([Run from a build](/deploy/run-from-a-build)); where a step is option-specific it says so.
...
- [ ] PostgreSQL 16+ running and accessible from the NeoBoard process
- [ ] TLS/SSL certificate configured on the reverse proxy
- [ ] Version pinned — an explicit image tag (e.g. `ghcr.io/…/neoboard:1.5.0`, **not** `:latest`) via `NEOBOARD_IMAGE` for Docker Compose or the manifest for Kubernetes, a release tag checkout for a build you run yourself — so deploys are reproducible and an upgrade cannot cross a major version unintentionally
- [ ] Resource limits set (CPU: 2 cores, memory: 1 GB recommended) — compose `deploy.resources`, Kubernetes `resources`, or a systemd `MemoryMax=`
- [ ] Log rotation configured (max 10MB, 3 files) — the compose files set the json-file driver; journald and the kubelet rotate on their own
- [ ] Backup strategy in place (see [Backup & Restore](/deploy/backup-restore))
- [ ] Health check configured (see [Monitoring](/deploy/monitoring))
...
If a deployment fails — the commands are for Docker Compose; under systemd, stop the unit, point it at the previous build and add `MIGRATE_ON_START=0` before starting; on Kubernetes, `kubectl set env deployment/neoboard MIGRATE_ON_START=0` and then `kubectl set image` to the previous tag. Steps 3 and 4 are the same everywhere.
...
2. **Restore the previous image.** Start it with `MIGRATE_ON_START=0`: the
   ...
   The shipped compose files (`docker-compose.prod.yml`, `docker-compose.prod-full.yml`)
   do not forward `MIGRATE_ON_START`, so prefixing `docker compose` with it has no
   effect and the container keeps the image's `1`. Add
   `MIGRATE_ON_START: ${MIGRATE_ON_START:-1}` under the `neoboard` service's
   `environment:` in the compose file you use first.
...
3. **If the database was migrated, restore from backup.** `--clean --if-exists`
   ...
   [Backup & Restore](/deploy/backup-restore)).
...
migrating. A build you run yourself has no such default — set it to `1` explicitly, as [Run from a build](/deploy/run-from-a-build) does, or the schema is never created.

The shipped compose files do not forward `MIGRATE_ON_START`, so prefixing
`docker compose` with it has no effect and the container keeps the image's `1`.
Add `MIGRATE_ON_START: ${MIGRATE_ON_START:-1}` to the `neoboard` service's
`environment:` first; then this boots without migrating:
...
that collides with it (see [Backup & Restore](/deploy/backup-restore)).
