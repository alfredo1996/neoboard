...
2. **Restore the previous image.** Start it with `MIGRATE_ON_START=0`: the
   ...
   worse state than it started. The shipped compose files forward
   `MIGRATE_ON_START` to the app container, so prefixing `docker compose` with it
   is enough.
...
forward `MIGRATE_ON_START` to the app container with the image's default of `1`,
so this boots without migrating:
