...
Both files pull the released image (`ghcr.io/alfredo1996/neoboard`) and fall back to building from source when no image is available. Pin a specific release with `NEOBOARD_IMAGE=ghcr.io/alfredo1996/neoboard:X.Y.Z`. Hosts with no internet access? [Air-gapped install](/deploy/air-gapped) lists what to mirror and what still needs a network.
