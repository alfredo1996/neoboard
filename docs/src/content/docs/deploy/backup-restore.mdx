...
3. **Set up the new server, starting only the database:**
   ...
   # The app migrates at boot by default (MIGRATE_ON_START=1), so starting it
   # now would create the schema on the empty database and the restore in
   # step 4 would collide with it. Start only PostgreSQL.
