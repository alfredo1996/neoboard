...
[Key rotation](/security/credential-encryption#how-to-rotate-zero-data-loss) re-encrypts the live database
...
If NeoBoard runs with the production stack (`docker-compose.prod-full.yml`), address the database through compose; the containers are named `<project>-postgres-1`, not `neoboard-postgres`:

```bash
C="docker compose --env-file docker/.env.production.local -f docker/docker-compose.prod-full.yml"
$C exec -T postgres pg_dump -U neoboard -d neoboard -Fc > neoboard_backup.dump
```

Replace `neoboard` after `-U` and `-d` if you changed `POSTGRES_USER` or `POSTGRES_DB`. `docker-compose.prod.yml` bundles no PostgreSQL, so bring-your-own-database installs use the host `pg_dump` recipe above against `DATABASE_URL`. CLI installs (`neoboard start`) name the containers `neoboard-postgres` and `neoboard-app`, and should use `neoboard db dump` / `neoboard db restore`.
...
If using the production stack's PostgreSQL (`docker-compose.prod-full.yml`):
...
# restore hits "already exists" and duplicate-key errors, because stopping the
# neoboard service stops the APP container — PostgreSQL keeps every row it had.
$C exec -T postgres pg_restore -U neoboard -d neoboard \
...
$C exec -T postgres dropdb -U neoboard --if-exists neoboard
$C exec -T postgres createdb -U neoboard neoboard
$C exec -T postgres psql -U neoboard -d neoboard < neoboard_backup.sql
...
3. **Set up the new server, starting only the database:**
   ```bash
   # Must hold the source's ENCRYPTION_KEY, NEXTAUTH_SECRET, API_KEY_HMAC_SECRET
   # and POSTGRES_PASSWORD. Never put these in docker/.env, which the CLI owns.
   cp .env.source docker/.env.production.local

   # The image boots with MIGRATE_ON_START=1 and the compose file does not pass
   # an override, so starting the app now would create the schema on the empty
   # database and the restore in step 4 would collide with it. Start only
   # PostgreSQL.
   docker compose --env-file docker/.env.production.local \
     -f docker/docker-compose.prod-full.yml up -d postgres
   ```

4. **Restore the database.** This compose file publishes no PostgreSQL port, so restore through the container:
   ```bash
   docker compose --env-file docker/.env.production.local \
     -f docker/docker-compose.prod-full.yml exec -T postgres \
     pg_restore -U neoboard -d neoboard --clean --if-exists < migration.dump
   ```

   Then start the app, which applies any migrations the dump predates:

   ```bash
   docker compose --env-file docker/.env.production.local \
     -f docker/docker-compose.prod-full.yml up -d
   ```
