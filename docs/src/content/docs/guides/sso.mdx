...
NeoBoard Enterprise supports OIDC-based single sign-on (SSO). Users can sign in with their organization's identity provider (Okta, Azure AD/Entra, Google Workspace, Keycloak, Auth0, etc.) instead of managing separate NeoBoard passwords.
...
1. An admin configures one or more OIDC providers in **Settings > Authentication**
...
- NeoBoard running with `NEOBOARD_EDITION=enterprise`
