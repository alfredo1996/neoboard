...
- [ ] `DATABASE_URL` points to the production PostgreSQL instance (bundled or BYO)
- [ ] `NEO4J_URI` set when using BYO Neo4j; bundled stack uses the default `bolt://neo4j:7687`
- [ ] `NEO4J_PASSWORD` generated (required for both bundled and BYO modes; compose refuses to start without it)
...
Tip: `neoboard env init` generates `ENCRYPTION_KEY`, `NEXTAUTH_SECRET`, and `NEO4J_PASSWORD` in one shot.
