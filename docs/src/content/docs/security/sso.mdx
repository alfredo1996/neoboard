...
2. Configured providers are registered with Auth.js, but the login page does not yet show SSO buttons; it currently renders only the email and password form
3. An SSO sign-in has to be started with a `POST` to `/api/auth/signin/<providerId>` that carries the CSRF token from `/api/auth/csrf` (a `GET` to that route returns "Unsupported action"). NeoBoard then redirects the user to the identity provider (IdP)
...
That's it — the provider is registered with Auth.js under the id `sso-env-oidc`. The login page does not yet render an SSO button for it.
...
| `OIDC_DISPLAY_NAME` | No | `SSO` | Provider display name (not yet shown on the login page) |
...
| `OIDC_ENFORCE_SSO` | No | `false` | Saved and reported, but not enforced yet. Password login stays available |
...
services:
  neoboard:
    image: ghcr.io/alfredo1996/neoboard:X.Y.Z # pin a release tag
    environment:
...
For organizations that need multiple SSO providers (e.g., Okta + Azure AD), use the Admin UI. The env-based provider and UI-based providers work together.
...
2. **Click "Add Provider"** and fill in the form:
   ...
   |-------|-------------|---------|
   | Display Name | Provider display name (not yet shown on the login page) | `Company SSO` |
...
4. **Set provisioning options:**
   ...
   - **Enforce SSO** — Saved with the provider, but not enforced yet: the login page still shows the password form, and any user who has a password can still sign in with it, admin or not. Users created automatically by SSO have no password, so they can only sign in through SSO. Users created with a password keep it, and NeoBoard has no way to remove it. Disabling a user blocks SSO sign-in too.

5. **Click "Add Provider"** to save

</Steps>

The provider is registered immediately and listed in Settings > Authentication. The login page does not yet render a button for it.
...
<Aside type="caution">
Google's OIDC tokens don't include a `groups` claim by default. All Google SSO users will get the **default role** (from their second sign-in; see [Role Mapping](#role-mapping)) unless you use Google Workspace directory groups with a custom claim mapper.
</Aside>

## Role Mapping

Role mapping is applied each time an existing user signs in, so group changes in the IdP take effect on their next sign-in. The exception is a user's very first SSO sign-in: it creates the account with the built-in defaults (Creator, with write access), not the mapped or default role. The mapped role normally takes effect from the second sign-in, provided `TENANT_ID` is `default`. With any other `TENANT_ID`, auto-provisioned users are created in tenant `default`, cut off from the instance's data, and the mapping never re-syncs ([#1791](https://github.com/alfredo1996/neoboard/issues/1791)).
...
| SSO button not showing | The login page does not render SSO buttons yet (see [How It Works](#how-it-works)). The public `/api/auth/sso-providers` endpoint returns only providers that are **Enabled** in Settings > Authentication. |
...
| "Access Denied: You do not have permission to sign in." (URL shows `?error=AccessDenied`) | The user doesn't exist in NeoBoard and auto-provisioning is turned off for this provider. Create the user manually first, or enable auto-provisioning. |
| Login loops back to /login | Ensure `NEXTAUTH_URL` is set correctly in your environment and matches the URL users access NeoBoard from. On HTTPS this loop is a known bug: Auth.js names the session cookie `__Secure-authjs.session-token`, and the request proxy does not read that name yet, so every browser session is sent back to `/login` ([#1792](https://github.com/alfredo1996/neoboard/issues/1792)). API keys are not affected. |
