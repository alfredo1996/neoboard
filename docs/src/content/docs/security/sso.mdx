...
Role mapping is applied each time an existing user signs in, so group changes in the IdP take effect on their next sign-in. The exception is a user's very first SSO sign-in: it creates the account with the built-in defaults (Creator, with write access), not the mapped or default role. The mapped role normally takes effect from the second sign-in.
