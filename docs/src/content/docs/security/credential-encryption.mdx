...
4. **Call the rotation endpoint** with an [API key](/security/api-keys) belonging to an **admin** user (Settings → API Keys). API keys carry no role of their own. On each request the endpoint checks the current role of the user who owns the key, so a creator's or reader's key returns `403`, and so does a key whose owner is no longer an admin.
   ...
   Use an API key for this call. A browser session cookie also works, sent **as a cookie** rather than as a Bearer token: `--cookie "__Secure-authjs.session-token=<value>"` when `NEXTAUTH_URL` is an `https://` URL, `--cookie "authjs.session-token=<value>"` when it is `http://`.
