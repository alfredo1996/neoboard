...
1. **Read the taste document first**: `.claude/skills/design-review/SKILL.md` for the rules, then `.claude/skills/design-review/reference.md` for the full tokens and palettes. Report in the Output Format below, not the reference's general critique format. It is the system, not aspiration — tokens, spacing rhythm, the typography scale that stops at `text-lg`, chart defaults, and the anti-pattern list.
