...
2. Check Docker state: `docker ps --format '{{.Names}}: {{.Status}}'`. If E2E will run, first remove this project's containers only — `docker rm -f $(docker ps -aq --filter 'name=neoboard-') 2>/dev/null` and `docker rm -f $(docker ps -aq --filter 'label=org.testcontainers=true') 2>/dev/null`. Never remove every container on the machine: other projects' containers (a kind cluster, for one) live there too. Then `docker compose up -d` and wait for healthchecks.
