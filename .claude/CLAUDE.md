...
- Allowed: query-language names (`cypher`, `sql` — a language is not a connector), **shape** checks that read the SDK's `$type` tag, the app's own metadata database under `app/src/lib/db/`, dev-only helpers under `app/src/lib/dev/` (#1905), and library package names (`@neo4j-nvl/*`, `@neo4j-cypher/*`).
- Where the shape guards come from depends on who bundles the file. Server-only code takes `isGraphNode` / `isGraphRelationship` / `isGraphPath` from `@neoboard/connection`. Anything the browser bundles takes them from `@neoboard/components/row-shapes` — that barrel pulls the drivers, and a client component importing it fails the build on `fs`, `net` and `tls`. The two are kept identical by `app/src/lib/__tests__/row-shape-mirror.test.ts` (#1925); import the subpath, never the `@neoboard/components` barrel, which pulls every chart.
- NOT allowed: duck-typing a value by its keys (#1925). `"labels" in v && "properties" in v` is a guess about one driver's output that a `jsonb` column also satisfies — read the tag.
- Enforced by two guards in `app/src/lib/__tests__/connector-agnostic.test.ts` (#1894): a name guard over `app/src` and `component/src`, and an export-surface guard over the `connection` and `connector-sdk` entry points. Both derive the forbidden names from the registry, so a new connector is covered from birth. **Both are at zero**: the baseline they ratcheted down from (297 names in 71 files, 10 exports) was driven out by epic #1893 and deleted in #1905. What remains is the permanent allowlist at the top of that file — each entry with the reason it is not connector knowledge, and mirrored in `.claude/hooks/check-boundaries.sh`, which must stay in step. Widen it only for a name that genuinely is not a connector fact, never to get a change through; the fix for a failing guard is to read the fact off the connector.
