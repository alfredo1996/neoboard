...
- Allowed: query-language names (`cypher`, `sql` — a language is not a connector), **shape** checks that read the SDK's `$type` tag (`isGraphNode` / `isGraphRelationship` / `isGraphPath` via `@neoboard/connection`), the app's own metadata database under `app/src/lib/db/`, and library package names (`@neo4j-nvl/*`, `@neo4j-cypher/*`).
