...
Single command from a fresh checkout — bundled Postgres + Neo4j + app:

```bash
git clone https://github.com/alfredo1996/neoboard.git
cd neoboard
npx @neoboard/cli env init      # generates ENCRYPTION_KEY, NEXTAUTH_SECRET, NEO4J_PASSWORD into .env
docker compose up -d            # boots app + postgres + neo4j (Docker Compose v2.20+)
# → http://localhost:3000
```

The root `docker-compose.yml` `include`s [`docker/docker-compose.prod.yml`](docker/docker-compose.prod.yml) — that file is the canonical production manifest.

#### Bring your own databases (BYO)

To point at an external Postgres and/or Neo4j, start only the `neoboard` service and override the connection URLs:
...
Because `depends_on.required` is `false` for the bundled DBs, naming just the `neoboard` service skips spinning them up.

#### Required environment variables

`docker compose up` fails fast with a clear error if any of these are missing:

| Variable          | Generated by        | Notes                                                                 |
| ----------------- | ------------------- | --------------------------------------------------------------------- |
| `ENCRYPTION_KEY`  | `neoboard env init` | **Losing this loses every stored connection credential.** Back it up. |
| `NEXTAUTH_SECRET` | `neoboard env init` | Rotating it signs every user out.                                     |
| `NEO4J_PASSWORD`  | `neoboard env init` | Required even in BYO mode (compose-level validation).                 |

See [`app/.env.example`](app/.env.example) for the full list (OIDC/SSO, bootstrap admin, logging, etc.).

#### Data lifetime warning

The bundled stack persists data in the named Docker volumes `pgdata`, `neo4j_data`, and `neo4j_logs`. **Deleting these volumes deletes all dashboards, connections, and users.** Schedule a backup before running `docker compose down -v`.
