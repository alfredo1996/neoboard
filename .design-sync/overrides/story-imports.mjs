// forked from design-sync lib/story-imports.mjs — kebab-case source files (bar-chart.tsx) → PascalCase exports (BarChart) so previews shim to the shipped global instead of re-bundling source. See exportedComponentFor.
...
function exportedComponentFor(p, exported) {
  ...
  // FORK (NeoBoard): source files are kebab-case (`bar-chart.tsx`) but exports
  // are PascalCase (`BarChart`). Map kebab→Pascal so component imports shim to
  // the shipped window.<GLOBAL> instead of re-bundling source (which pulls CJS
  // deps that break, e.g. CirclePackingChart). Non-component kebab files
  // (`chart-utils` → `ChartUtils`, `use-toast` → `UseToast`) won't be in
  // `exported`, so they still bundle — same safety as the stock matcher.
  const toPascal = (s) => s.replace(/(^|-)([a-z0-9])/g, (_, __, c) => c.toUpperCase());
  const pf = toPascal(file);
  if (exported.has(pf)) return pf;
  if ((file === 'index' || file === dir) && exported.has(toPascal(dir))) return toPascal(dir);
  return null;
}
