...
- The release workflow signs and scans the image under the lowercased name it pushes, so a repository owner with capital letters no longer turns the release red (#1780)
...
- The README and docs site state the 16 chart types and 7 parameter types the editor offers, instead of 18 chart types and Number, Boolean and Slider parameters it does not (#1782)
- The public docs no longer claim behaviour the code does not have: SSO login buttons and Enforce SSO, the role a first SSO sign-in gets, what creators and readers can do with connections and queries, Form-only writes, envelope encryption, per-IP rate limiting, query priority tiers, a `MIGRATE_ON_START` prefix the compose files never forward, CLI flags and output, and chart options that do not exist. A docs guard fails if any of the security claims comes back (#1790)
- Test and CI hygiene: CI runs for PRs confined to `connector-sdk/` or `docker/`; tests run shuffled; every package has a coverage floor; E2E collects the server-side coverage it declared; test files are linted; SonarCloud no longer excludes measurable code; ratchets, hooks and test collection that could pass silently now fail; route tests assert the tenant filter each handler passed; the connector-SDK row-limit conformance case has a lower bound; `connection/` actually typechecks; `component/` drops five unused runtime dependencies; admin routes drop a `canWrite` check that could never fail; build artefacts are untracked; docs link checks derive the repo owner from the compose image and fail when a pattern matches nothing (#1627, #1630, #1610, #1612, #1611, #1613, #1632, #1633, #1634, #1607, #1631, #1628, #1595, #1297, #1578, #1689, #1781)
