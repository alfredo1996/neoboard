...
- `npm audit fix` applied, taking the dependency tree from **15 advisories (1 low, 4 moderate, 10 high) to 9 (4 moderate, 5 high)**. Lockfile only — no direct dependency versions moved, so nothing about the application changed. The remaining nine collapse to two direct dependencies, `@neo4j-nvl/react` and `drizzle-kit`, and **neither has a forward fix**: both are already on the newest published version, and npm's "fix available" for each is a downgrade to an older major (NVL 1.2.1 → 1.0.0, drizzle-kit 0.31.10 → 0.18.1). Running `npm audit fix --force` would therefore regress both rather than patch anything
