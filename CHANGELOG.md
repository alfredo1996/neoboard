...
- The public docs no longer claim behaviour the code does not have: SSO login buttons and Enforce SSO, the role a first SSO sign-in gets, what creators and readers can do with connections and queries, Form-only writes, envelope encryption, per-IP rate limiting, query priority tiers, a `MIGRATE_ON_START` prefix the compose files never forward, CLI flags and output, and chart options that do not exist. They now name two known auth bugs, SSO users landing in tenant `default` (#1791) and HTTPS sessions redirecting to `/login` (#1792). A docs guard fails if the most load-bearing of these claims come back (#1790)
