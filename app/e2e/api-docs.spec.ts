...
test.describe("API docs — /api/docs and /api/openapi.json", () => {
  ...
  test("GET /api/openapi.json does not set wildcard CORS header", async ({
    request,
  }) => {
    // Wildcard CORS was removed in #689 (security hardening).
    // The OpenAPI endpoint no longer sets access-control-allow-origin.
    const res = await request.get("/api/openapi.json");
  ...
  test("/api/docs page renders Swagger UI in browser", async ({
    authPage,
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "NeoBoard API" }),
  ...
  test("/api/docs shows all resource sections", async ({ authPage, page }) => {
    ...
    await expect(
      page.getByRole("link", { name: "Query", exact: true }),
    ).toBeVisible();
    await expect(
