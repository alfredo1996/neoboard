...
describe("resolveClickAction", () => {
  ...
  it("uses _clickedValue when sourceField is empty string", () => {
    ...
    const result = resolveClickAction(widget, {
      _clickedColumn: "department",
      _clickedValue: "Engineering",
    });
    expect(result).toEqual({
      setParameter: {
        parameterName: "dept",
  ...
  it("returns null when clicked column is not in clickableColumns", () => {
    ...
    const result = resolveClickAction(widget, {
      _clickedColumn: "status",
      _clickedValue: "Active",
    });
    expect(result).toBeNull();
  });
...
describe("resolveClickActions", () => {
  ...
  it("matches table cell click to correct rule by triggerColumn", () => {
    ...
    const result = resolveClickActions(widget, {
      _clickedColumn: "name",
      _clickedValue: "Alice",
    });
    expect(result).toEqual({
  ...
  it("matches second rule when clicking corresponding column", () => {
    ...
    const result = resolveClickActions(widget, {
      _clickedColumn: "email",
      _clickedValue: "alice@example.com",
    });
    expect(result).toEqual({
      setParameter: {
        parameterName: "selectedEmail",
  ...
  it("returns null when rule has no parameterMapping for set-parameter type", () => {
    const widget = makeWidget({
      settings: {
        clickAction: {
          type: "set-parameter",
          rules: [{ id: "r1", triggerColumn: "name", type: "set-parameter" }],
        },
      },
    });
    const result = resolveClickActions(widget, {
    ...
    const action: ClickAction = {
      ...
      rules: [
        ...
        },
        {
          ...
          triggerColumn: "email",
          type: "set-parameter",
          parameterMapping: { parameterName: "y", sourceField: "email" },
        },
      ],
    };
    expect(deriveClickableColumns(action)).toEqual(["name", "email"]);
