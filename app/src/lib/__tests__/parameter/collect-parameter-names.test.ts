...
import {
  ...
  getWidgetParameterNames,
  aggregateClickActionParamNames,
  buildParameterSourceMap,
        ...
        widgets: [
          {
            ...
            connectionId: "c1",
            query:
              "SELECT * FROM users WHERE name = $param_selectedName AND dept = $param_dept",
            settings: {},
          },
        ],
        gridLayout: [{ i: "w1", x: 0, y: 0, w: 4, h: 3 }],
    ...
    expect(
      getWidgetParameterNames({
        ...
        settings: {
          clickAction: {
            type: "set-parameter",
            parameterMapping: { parameterName: "foo", sourceField: "x" },
          },
        },
      }),
    ).toEqual(["foo"]);
            ...
            rules: [
              {
                id: "r1",
                type: "set-parameter",
                parameterMapping: { parameterName: "a", sourceField: "x" },
              },
              {
                id: "r2",
                type: "set-parameter",
                parameterMapping: { parameterName: "b", sourceField: "y" },
              },
            ],
          },
        },
...
describe("aggregateClickActionParamNames", () => {
  ...
  it("returns top-level parameter name when set", () => {
    expect(aggregateClickActionParamNames(true, "selected", [])).toEqual([
      "selected",
    ]);
  });

  it("trims whitespace from top-level parameter name", () => {
    expect(aggregateClickActionParamNames(true, "  name  ", [])).toEqual([
      "name",
    ]);
  });
  ...
  it("skips empty top-level parameter name", () => {
    expect(
      aggregateClickActionParamNames(true, "", [
  ...
  it("deduplicates names", () => {
    expect(
      aggregateClickActionParamNames(true, "dup", [
  ...
  it("handles rules without parameterMapping", () => {
    expect(
      aggregateClickActionParamNames(true, "top", [
